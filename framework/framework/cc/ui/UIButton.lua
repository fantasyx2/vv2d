...
function UIButton:ctor(events, initialState, options)
    ...
    self.scale9CapInsets = options and options.scale9CapInsets
...
function UIButton:updateButtonImage_()
    ...
    if image then
            ...
            if "table" == type(image) then
                for i,v in ipairs(image) do
                    if self.scale9_ then
                        self.sprite_[i] = display.newScale9Sprite(v,nil,nil,nil,self.scale9CapInsets)
                ...
                if self.scale9_ then
                    self.sprite_[1] = display.newScale9Sprite(image,nil,nil,nil,self.scale9CapInsets)
                    if not self.scale9Size_ then
        ...
        for i,v in ipairs(self.sprite_) do
            ...
            v:setPosition(sz.width/2, sz.height/2)
        end
...
--]]
    ...
    for _, l in pairs(self.labels_) do
        ...
        l:setPosition(sz.width/2+ox, sz.height/2+oy)
    end
end
